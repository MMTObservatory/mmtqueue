...
from .utilities import roundTime
from .utilities import get_key
from .utilities import set_score

from multiprocessing import Pool

# Removing SequentialScheduler from here.
__all__ = ['ObservingBlock', 'TransitionBlock', 'Schedule', 'Slot', 'Scheduler',
           'PriorityScheduler', 'Transitioner', 'Scorer']
# __all__ = ['ObservingBlock', 'TransitionBlock', 'Schedule', 'Slot', 'Scheduler',
...
class Scorer(object):
    ...
    # time_resolution was original 1*u.minute, trying 10*u.minutes
    def create_score_array(self, time_resolution=10*u.minute):
        """
        ...
        """
        # Starting multiprocessing approach  2018-02-23  JDG.
        ...
        for i, block in enumerate(self.blocks):
            ...
            if block.constraints:
                for constraint in block.constraints:
                    if False:
                        # print("Processing block constraint: ", repr(constraint), \
                        #      ", target: ", repr(block.target), ", times: ", repr(times))
                        print("Processing block constraint: ", repr(constraint), \
                              ", target: ", repr(block.target), ", start_time: ", times[0])
                    if multiprocessing:
                        pool_inputs = []
                        for t in times:
                            pool_inputs.append((self.observer,
                                                block.target,
                                                [t]))
                        applied_score = pool.starmap(constraint, pool_inputs)
                        for score in applied_score:
                            score_array[i] *= score
                    else:
                ...
                if False:
                    print("applied_score: ", repr(applied_score))
  
            for constraint in self.global_constraints:
                if multiprocessing:
                    for target in self.targets:
                        if False:
                            print("Processing global constraint: ", repr(constraint), \
                              ", target: ", repr(block.target), ", start_time: ", times[0])
                        pool_inputs = []
                        for t in times:
                            pool_inputs.append((self.observer,
            ...
            if False:
                print("Global Score target: ", repr(score_array))
...
class Schedule(object):
     ...
     def insert_slot(self, start_time, block):
        ...
        Traceback (most recent call last):
          File "./mmtscheduler.py", line 1444, in <module>
            main(args)
          File "./mmtscheduler.py", line 1423, in main
            run_scheduler()
          File "./mmtscheduler.py", line 1236, in run_scheduler
            s = seq_scheduler(blocks, sequential_schedule)
          File "/home/mmtqueue/src/QueueScheduler2.0/mmtqueue/scheduling.py", line 549, in __call__
            schedule = self._make_schedule(copied_blocks)
          File "./mmtscheduler.py", line 690, in _make_schedule
            self.schedule.insert_slot(trans.start_time, trans)
          File "/home/mmtqueue/src/QueueScheduler2.0/mmtqueue/scheduling.py", line 358, in insert_slot
            if (block.duration - self.slots[slot_index].duration) > 1*u.second:
         UnboundLocalError: local variable 'slot_index' referenced before assignment
         """
         
         # New check for above error.  JDG  
...
class Scheduler(object):
     ...
     def __init__(self, constraints, observer, transitioner=None,
                  gap_time=30*u.min, time_resolution=20*u.second):
         """
         Parameters
         ----------
...
class PriorityScheduler(Scheduler):
    ...
    def _make_schedule(self, blocks):
        ...
        print("time_resolution: ", repr(time_resolution))
